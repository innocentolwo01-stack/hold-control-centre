...
import {
  Activity, Bell, Boxes, Building2, CalendarRange, CircleDollarSign, Gift, Globe2, LayoutDashboard,
...
const items = [
  ...
  ['Market catalogue', '/dashboard/market-catalogue', Globe2],
  ...
  return (
    <div className="dashboard-shell">
      <aside className="sidebar">
        ...
        <nav>{items.map(([label, href, Icon]) => { const active = href === '/dashboard' ? pathname === href : pathname.startsWith(href); return <Link key={`${label}-${href}`} href={href} className={active ? 'active' : ''}><Icon size={18} /><span>{label}</span></Link>; })}</nav>
